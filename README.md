...
- **Кнопка «Обновить»** на каждый договор — принудительный опрос вне интервала
  (например, сразу после пополнения).
- Атрибуты: оператор, номер договора, лицевой счёт, статус, число транспондеров,
  время актуальности данных и **ссылка на пополнение** (`topup_url`).
...
| `transponders` | —               | `1`                  |
| `topup_url`    | ссылка оплаты   | ссылка оплаты        |
| Бонусы         | —               | сенсор `bonus`, `400`|

Плюс кнопка `button` «Обновить» на каждый договор (принудительный опрос).
Атрибут `topup_url` содержит персональную ссылку оплаты (с email/телефоном и
подписью) — она пишется в recorder; при желании исключите её через `exclude`.
